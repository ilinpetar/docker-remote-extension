...
func main() {
	...
	router.GET("/ssh-config", sshConfig)
...
func sshConfig(ctx echo.Context) error {
	response, err := ParseSshConfig()
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, HTTPMessageBody{Message: "error"})
	}
